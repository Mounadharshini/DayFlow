...
export default function Attendance() {
  ...
  // Find today's attendance record from database array
  const todayRecord = records.find((r) => r.date === todayStr || r.date === localTodayStr);
  ...
  const handleCheckIn = async () => {
    ...
    setError('');
    try {
      const newRec = await api.checkIn(token);
  ...
  const handleCheckOut = async () => {
    ...
    setError('');
    try {
      const updatedRec = await api.checkOut(token);
  ...
  const filteredRecords = records.filter(r => {
    if (range === 'day') return r.date === todayStr || r.date === localTodayStr;
    return true;
  });
  ...
  return (
    <div className="container" style={{ paddingBottom: 60 }}>
      ...
      {/* TODAY'S ATTENDANCE STATUS CARD WITH LIVE CHECK-IN & CHECK-OUT TIMESTAMPS */}
      <div style={{
        background: '#ffffff',
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 8, flexWrap: 'wrap' }}>
              <span className={`badge ${todayRecord?.checkIn ? (todayRecord.status === 'Half-day' ? 'badge-pending' : todayRecord.status === 'Leave' ? 'badge-rejected' : 'badge-present') : 'badge-pending'}`}>
                {todayRecord?.checkIn 
                  ? (todayRecord.checkOut ? `Checked Out (${todayRecord.status})` : `Checked In (${todayRecord.status})`) 
                  : 'Not Checked In'}
              </span>
              <span style={{ fontSize: 13, color: '#2b1b12', fontWeight: 600 }}>
                Check-In Time: <strong style={{ color: todayRecord?.checkIn ? '#9c6137' : '#7a6758' }}>{todayRecord?.checkIn || '-- : -- : --'}</strong>
              </span>
              <span style={{ fontSize: 13, color: '#2b1b12', fontWeight: 600 }}>
                Check-Out Time: <strong style={{ color: todayRecord?.checkOut ? '#dc2626' : '#7a6758' }}>{todayRecord?.checkOut || '-- : -- : --'}</strong>
              </span>
              <span style={{ fontSize: 13, color: '#9c6137', fontWeight: 800 }}>
                Log Duration: {todayRecord?.workHours ? `${todayRecord.workHours} hrs` : '0.0 hrs'}
              </span>
            </div>
          </div>

          {/* CHECK IN AND CHECK OUT SIDE-BY-SIDE ON THE EXACT SAME LINE */}
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 10, flexWrap: 'nowrap', whiteSpace: 'nowrap' }}>
            <button 
            ...
            <tbody>
              {filteredRecords.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={{ fontSize: 13, fontWeight: 600, color: '#9c6137' }}>{r.checkIn || '—'}</td>
                  <td style={{ fontSize: 13, fontWeight: 600, color: '#dc2626' }}>{r.checkOut || '—'}</td>
                  <td style={{ fontWeight: 700, color: '#2b1b12' }}>{r.workHours ? `${r.workHours} hrs` : '8.0 hrs'}</td>
                  <td>
