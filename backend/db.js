  ...
  prepare: (sqlStr) => {
    ...
    return {
      ...
      all: (...params) => {
        ...
        // Employee Attendance Query (SELECT * FROM attendance WHERE userId = ?)
        if (cleanSql.includes('FROM attendance') && !cleanSql.includes('JOIN')) {
          const [uId, minDate] = params;
          let rows = data.attendance.filter(a => String(a.userId) === String(uId));
          if (minDate && cleanSql.includes('date >= ?')) {
            rows = rows.filter(a => a.date >= minDate);
          }
      ...
      run: (...params) => {
        if (cleanSql.includes('INSERT INTO users')) {
          ...
          if (cleanSql.includes('isEmailVerified')) {
            u = {
              ...
              pf: 36000,
              tax: 60000,
              profilePicture: (params[6] && typeof params[6] === 'string' && (params[6].startsWith('http') || params[6].startsWith('data:image/'))) ? params[6] : '',
              documents: '[]',
        ...
        // PROFILE UPDATE HANDLERS (EXPLICIT MATCHING TO PREVENT MISMATCHES)
        if (cleanSql.includes('UPDATE users SET name = ?, phone = ?, address = ?, profilePicture = ? WHERE id = ?')) {
          const [name, phone, address, pic, id] = params;
          const u = data.users.find(x => String(x.id) === String(id));
          if (u) {
            if (name !== undefined) u.name = name;
            if (phone !== undefined) u.phone = phone;
            if (address !== undefined) u.address = address;
            if (pic !== undefined) u.profilePicture = pic;
            saveDB();
          }
          return { changes: 1 };
        }
        ...
        if (cleanSql.includes('UPDATE users SET profilePicture = ? WHERE id = ?')) {
          const [pic, id] = params;
          const u = data.users.find(x => String(x.id) === String(id));
          if (u) {
        ...
        if (cleanSql.includes('UPDATE users SET documents = ? WHERE id = ?')) {
          ...
          const u = data.users.find(x => String(x.id) === String(id));
          if (u) {
            u.documents = typeof docs === 'string' ? docs : JSON.stringify(docs);
            saveDB();
          }
          return { changes: 1 };
        }
        ...
        if (cleanSql.includes('UPDATE users SET name = ?, phone = ?, address = ?, department = ?') || (cleanSql.includes('UPDATE users SET name = ?') && params.length >= 14)) {
          ...
          const u = data.users.find(x => String(x.id) === String(targetId));
          if (u) {
            if (name) u.name = name;
            if (phone !== undefined) u.phone = phone;
            if (address !== undefined) u.address = address;
